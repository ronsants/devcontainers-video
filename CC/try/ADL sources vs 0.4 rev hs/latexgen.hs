...
--   lshow = show
  ...
  instance LATEX Context where
   ...
   ltshow cname typ ctx@(Ctx nm on isa world dc ms cs)
    = (chain "\n". filter (not.null))
...
--       capss (Cl g cls) = ["\\super{"++lshow g++"}{"++lshow s++"}" | s<-map root cls]
       ...
       showPats (pat@(Pat nm rs gen pms cs):dc) dejavu
        = if language==Dutch then
           latexSection (nm) (nnm)         ++
           latexFigure (latexCenter ("  \\includegraphics[scale=0.5]{"++cname++"_"++nnm++".png}")++
                        "\n\\caption{Structuur van "++nm++"}\n\\label{fig:"++nnm++"}")  ++ "\n" ++
           (if null (rules pat) then "Geen regels voor "++nm++"." else
            showRestrictions (rules pat) 1 dejavu                                       ++ "\n")++
           (if null (specs pat) then "" else contDef cname typ pat++ "\n\n")                    ++
           showPats dc (dejavu++signatures (rules pat))
          else if language==English then
           latexSection (nm) (nnm)         ++
          ...
          where
           ...
           showRestrictions (r:rs) i dejavu
            = (if language==Dutch then "Uitgangspunt " else "Requirement ")++show (nr r)++
               ":\n\\begin{quote}\n"++explain r++"\n\\end{quote}\n"++
              (if null ms then "" else
               (if language==Dutch
                then "Om dit uitgangspunt te kunnen formaliseren, introduceren we de volgende "++
                     (if length ms==1
                      then (let [m]=ms in if isFunction m then "functie." else "relatie.")
                      else show (length ms) ++ " signaturen.")
                else "In order to formalize this requirement, we introduce the following "++
                      if length ms==1
                      then (let [m]=ms in if isFunction m then "function." else "relation.")
                      else show (length ms) ++ " signatures.")++"\n\n"++
               chain "\n\n" (map lshow ms) ++"\\\\\n\n")++
              (if language==Dutch
               then "Het uitgangspunt vereist dat aan de volgende regel op ieder moment is voldaan:"
               else "The requirement is represented by the following rule:")++
              "\n\\label{rel:"++nnm++" "++show i++"}\n"++
              "\\[" ++ ltshow cname typ (normRule lub r) ++"\\]\n"++
--              lschema cname typ (Pat (nm++" "++show i) [normRule lub r] [] [] []) ++
              (if language==Dutch then
              ...
              where ms = sort' name (rd [m|m<-signatures r, not (m `elem` dejavu)])
  ...
  ltglos cname typ ctx@(Ctx nm on isa world dc ms cs)
    = (chain "\n". filter (not.null))
      (if language==Dutch then 
...
\begin{schema}{Workflow}
  ...
  \signature{in}:\concept{Step}\fun\concept{Procedure}\\
  \signature{follows}:\concept{Step}\rel\concept{Step}\\
  \signature{performs}:\concept{Group}\pinj\concept{Step}\\
\where
  \signature{follows}\comp\signature\subseteq\signature{in}
--flip is \inv
\end{schema}
...
lpattern gets the complete typology of the context, in order to produce the right generic concepts (powersets).
   ...
   ltshow cname typ pat@(Pat nm rs gen pms cs)
    = if language==Dutch then
       ...
       latexSubsection ("Regels in " ++ nm) ("restrictions"++nnm)                          ++
          chain "\n\n" [lschema cname typ (Pat (nm++" "++show i) [normRule lub r] [] [] [])| (i,r)<-zip [1..] (rules pat)] ++
       latexSubsection ("Context definities in " ++ nm) ("defs"++nnm)++contDef cname typ pat
      else if language==English then
       latexSection (nm) (filter isAlphaNum nm)                                ++
       latexFigure (latexCenter ("  \\includegraphics[scale=.3]{"++cname++"_"++nnm++".png}")++
                    "\n\\caption{Structure of "++nm++"}\n\\label{fig:"++nnm++"}")                ++
       latexSubsection ("Rules in " ++ nm) ("restrictions"++nnm)                          ++
          chain "\n\n" [lschema cname typ (Pat (nm++" "++show i) [normRule lub r] [] [] [])| (i,r)<-zip [1..] (rules pat)] ++
       latexSubsection ("Context definitions in " ++ nm) ("defs"++nnm)++contDef cname typ pat
      else ""
      where nnm = clname nm

  contDef cname typ pat@(Pat nm rs gen pms cs)
   = lschema cname typ (Pat (nm++" "++show (length (rules pat)+1)) [Ru 'E' (F [Tm m True]) pos expr "" sgn nr| Gc pos m expr sgn nr<-specs pat] [] [] [])
  ...
  instance LATEX Signature where
   lshow mm@(Sgn _ _ _ _ _ _ _ _ _)
    = if language==Dutch then
              ...
              (if null ps then "" else
               if null ([Uni,Tot]>-ps) -- zo ja dan is het een functie
                then if null pfs
                     ...
                     else "Deze functie is "++commaEn [lshow p| p<-pfs]++".\\\\\n"
                else "Deze relatie is " ++ commaEn [lshow p| p<-ps]++".\\\\\n") ++
              (if null cs
               then "Betekenis: "++applyM mm ("$\\langle"++lshow(source mm)++"\\rangle$") ("$\\langle"++lshow(target mm)++"\\rangle$")
              ...
              (if null ps then "" else
               if null ([Uni,Tot]>-ps) -- zo ja dan is het een functie
                then if null pfs
                     ...
                     else "This function is "++commaEng "and" [lshow p| p<-pfs]++".\\\\\n"
                else "This relation is " ++ commaEng "and" [lshow p| p<-ps]++".\\\\\n") ++
              (if null cs
               then "Meaning: "++applyM mm ("$\\langle"++lshow(source mm)++"\\rangle$") ("$\\langle"++lshow(target mm)++"\\rangle$")
...
{- if used in Zed context, more specific arrows are:
  ...
  instance LATEX Expression where
   lshow (Tm m c) = idName m
   lshow (Tf m c) = "\\inv"++idName m
   lshow (F ts)  = chain "\\compose" (map (lshow) ts)
   lshow (Fu fs) = chain "\\cup" (map (lshow) fs)
   lshow (Fi fs) = chain "\\cap" (map (lshow) fs)

  instance LATEX Rule where
   ltshow cname typ (Gc pos m expr sgn nr) = ltshow cname typ (Ru 'E' (F [Tm m True]) pos expr "" sgn nr)
   ltshow cname typ r = ltshow cname typ (assemble r)
   lshow (Ru 'I' antc pos cons expla sgn nr) | fEmpty antc = lshow cons
                                             | fEmpty cons = "\\not("++lshow antc++")"
                                             | otherwise   = lshow antc++"\\subseteq"++lshow cons
   lshow (Ru 'E' defd pos expr expla sgn nr) = lshow defd++"="++lshow expr
  ...
  instance LATEX Restriction where
    ...
    lshow (Not rs) = "\\not "++lshow rs
    lshow (Rel lhs (Tm m compl) rhs d c)
         = (if compl then "" else "\\not (")++
           lshow (Funs d left)++"\\ "++lshow m++"\\ "++lshow (Funs c right)++
           (if compl then "" else ")")
           where left = [m| t<-lhs, m<-mors t]; right = [m| t<-reverse rhs, m<-mors t]
    lshow (Rel lhs (Tf m compl) rhs d c)
         = (if compl then "" else "\\not (")++
           lshow (Funs c left)++"\\ "++lshow m++"\\ "++lshow (Funs d right)++
           (if compl then "" else ")")
